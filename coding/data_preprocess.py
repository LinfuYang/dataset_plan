...
class data_pre:
    ...
    def time_pre(self):
        ...
        # 如果观测值和观测置信率都是0
        # drop_index = list(self.df_data[self.df_data['% Observed'] == 0 and self.df_data['Lane 1 Flow (Veh/5 Minutes)'] == 0].index)
        ...
        # print(list(((self.df_data['Lane 1 Flow (Veh/5 Minutes)'].values).reshape(1,-1))[0]))

        self.df_data['Lane 1 Flow (Veh/5 Minutes)'] = imp.fit_transform((self.df_data['Lane 1 Flow (Veh/5 Minutes)'].values).reshape(-1, 1))



# 目标函数点
df_0 = pd.read_csv('../data_merge/M_point.csv')
data_pre(df_0)
...
df_1 = pd.read_csv('../data_merge/M_point_1.csv')
data_pre(df_1)
comu = ['NSM', 'day_of_week', 'Lane 1 Flow (Veh/5 Minutes)']
df_1[comu].to_csv('../ending_dataset/data_774671.csv', index=False)

df_2 = pd.read_csv('../data_merge/M_point_2.csv')
data_pre(df_2)
comu = ['NSM', 'day_of_week', 'Lane 1 Flow (Veh/5 Minutes)']
